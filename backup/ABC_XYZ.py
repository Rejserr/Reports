...
output_dir = r"C:\Users\mladen.lackovic\Documents\Reports\2025\ABC_XYZ_Results"
...
try:
    ...
    plt.bar(range(len(sorted_items)), sorted_items['Total Turnover'], color='skyblue')
    plt.plot(range(len(sorted_items)), sorted_items['Cumulative %'], 'r-', marker='o', markersize=3)
    plt.axhline(y=80, color='g', linestyle='--', label='80% threshold (A items)')
    plt.axhline(y=95, color='orange', linestyle='--', label='95% threshold (B items)')
    plt.xlabel('Items ranked by turnover')
    plt.ylabel('Turnover / Cumulative percentage')
    plt.title('ABC Analysis - Pareto Chart')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'abc_pareto.png'))
    plt.close()
